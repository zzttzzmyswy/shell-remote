...
| # | 点 | 状态 | 证据 |
|---|---|---|---|
| 41 | 逐帧 binary+头{len,seq,flags} | ✔ | **帧头字段显式化已做**（第 52 轮：desktop:video frag 带消息级 `seq` + `flags`）；**relay 侧 seq 丢帧检测已做**（第 55 轮：`desktop_proto` 统计）；**binary 帧格式规格已落地**（第 67 轮：`proto` `encode_bin_frame`/`decode_bin_frame`——`[flags:u8][seq:u32 LE][len:u32 LE][payload]`，`BIN_FRAME_FLAG_KEY`/`INIT`，len 双重校验）；**relay 消费侧已落地**（第 68 轮：`Message::Binary` 分支 → `route_bin_desktop_frame`——init set_init / frag push_frag+congested / seq 检测+开销统计）；**agent 发送侧已切换**（第 69 轮：relay register 响应带 `desktop_binary: true` 能力告知 → agent `RelayClient.desktop_binary` 解析 → WS 上行 desktop:video 改发 binary 帧（ctrl text 数组 + media 逐帧 binary，`encode_bin_frame`；老 relay 无该字段 → 保持 JSON+base64 兼容回退；WS 失败仍回退 HTTP 批量 JSON）；测试 `test_handle_register_response_parses_desktop_binary`；**端到端实测**：agent→relay binary 全链路——relay recv 195 帧 / video-bin pushed(frag) 97 / 浏览器 WS 收完整 fMP4（ftyp=1+moof=29），4-top 动态基准 PASS（fps 30.0 满帧 / bitrate 670kbps），**0% 协议膨胀** vs 此前 JSON base64 ≈33%）。**#41 全链路闭环**（agent 编码端 binary → relay 消费端直转 → 浏览器 WS 原样 fMP4），老 relay/老 agent 双兼容 |
| 42 | seq 真实丢帧 | ✔ | desktop.js:_handleMoof seqn gap |
| 43 | relay 二进制直转 | ✔ | **协议开销观测子集已做**（第 53 轮：relay `SharedState.desktop_proto` 累计 desktop:video base64 编码/解码字节——量化 JSON 传输膨胀 ≈33%，为二进制直转决策提供 ROI 数据）；**admin overview 展示已做**（第 56 轮：每会话 `desktop_proto` 含 encoded_bytes/decoded_bytes/dropped_frames）；**二进制直转已实现**（第 69 轮 #41 闭环覆盖：relay `Message::Binary` 分支 → `route_bin_desktop_frame` **对 binary 帧 payload 原样字节直转** `set_init`/`push_frag`（不重新 base64 编码/解码），`desktop_proto` 统计 `encoded==decoded`（0% 膨胀）；老 agent JSON 路径的 base64 解码保留兼容）。**#43 闭环**（直转核心由 #41 全链路实现，观测/展示/直转三件套齐） |
| 44 | 老版本兼容/capability | ◐ | **capability 协商最小子集已做**（第 51 轮：agent 注册消息带 `capabilities` 数组（codec/后端/桌面功能声明）→ relay `SessionInfo.capabilities` + `update_capabilities`/`get_capabilities` → admin overview 展示——老版本 agent 不带保持空，浏览器可据此协商；测试 `test_register_capabilities_roundtrip`）；**声明真实性已做**（第 64 轮：`build_capabilities()` 按**平台 + 编译 feature + 运行时探测**真实声明——codec 按 feature（h264 恒在 / vp9·av1 按编译）、backend 按平台（Linux 声明 x11 + wayland 仅 `feature="wayland"` 且 `WAYLAND_DISPLAY` 可达；Windows 才声明 gdi/dxgi——修复此前 Linux agent 硬编码声明 `backend:gdi/dxgi` 不实）；测试 `test_build_capabilities_platform_accurate`/`test_build_capabilities_wayland_detection`（wayland 设/不设 env 双分支），端到端 admin overview 实测 Linux capabilities=`codec:h264,vp9,av1,backend:x11,desktop:*` 无 gdi/dxgi）；**老版本二进制兼容仍远期**（依赖 #41 线协议整块） |
...
| 127-128 | 捕获内存池/行拷贝 SIMD | ◐ | **像素转换 SIMD 已做**（第 61 轮：`color.rs` BGRA→I420 的 Y 平面走 SSSE3/SSE4.1 128 位内核（shuffle 提通道 + i32 乘加 + 饱和打包，4 像素/拍，行尾标量补齐），U/V 子采样网格行内不连续保持标量；`bgra_to_i420_with_matrix` 运行时 `is_x86_feature_detected!("sse4.1")` dispatch，非 x86/无特性回退标量；与标量参考**逐字节一致**（测试强制：BT.601/BT.709/padding stride/非 4 倍数行尾/全色极端 5 组）；bench：1080p 转换 4.30ms vs 标量 8.03ms = **1.9x**（手动 `cargo test --release -- --ignored simd_bench_1080p`）——行拷贝/像素转换循环不再依赖 LLVM 自动向量化，显式 SIMD 落地；**捕获内存池第一块已做**（第 77 轮：`FramePool` buffer 池化组件——预先分配复用 `Vec<u8>`，`acquire`/`release` 借出归还（cap 保留）、`prealloc` 启动摊分、`stats(复用/新分配)` 可观测命中率、池上限防膨胀；3 单测：同尺寸循环借出仅首轮新分配 / 池容量封顶 / 大尺寸不硬塞新分配）；**第二块已做**（第 78 轮：`pooled_frame` 接入 pipeline——合成帧（静态 IDR/reqkey 重编 `mod.rs:1133`）bgra 改从池借出拷贝、动态帧转换后 `release` 归还（`mod.rs:1224`），预分配 cfg 尺寸 buffer 启动摊分；单测 `test_pooled_frame_allocates_from_pool`（预分配命中复用/无新分配）；4-top 基准 PASS fps 30.0 无回归）；**第三块已做**（第 79 轮：**捕获侧池注入环形闭环**——`FrameSource::attach_pool` 默认 no-op，X11Source 挂池（`bgra_from_pixels` 从 `pool.acquire` 借 bgra），`ThreadedFrameSource::spawn_with_max_fps` 注入池（线程内 `inner.attach_pool`），pipeline 池改 `Arc` 共享（capture 借出 → main 归还 → capture 复用环形；pool.acquire 无 buffer 自分配兜底）；全量 `cargo test` **417 通过** + 4-top 基准 PASS（fps 30.0 / bitrate 670，捕获池化无回归）——**#127 内存池三件套闭环**（FramePool 组件 + 编码链接入 + 捕获侧环形） |
...
| 132-134 | Wayland/首帧/缩放 | ◐ | **代码级就绪 + 集成路径执行（第 76 轮）**：`wayland.rs`（397 行完整实现：xdg-desktop-portal ScreenCast → PipeWire → gstreamer `pipewiresrc` → appsink）**编译通过**（`cargo build --no-default-features --features wayland`，zbus+gstreamer pkg-config 就绪）；本机搭 Wayland 测试栈（weston headless + xdg-desktop-portal + portal-wlr，gst-plugin-pipewire 已装）——headless weston 起 socket + **portal-wlr 注册 ScreenCast**（日志确认）→ agent `--desktop-capture wayland` **portal 流程正常执行**（注册/desktop:start/WS uplink 无崩溃无错误）。**完整捕获运行时验证阻塞**：headless weston 无真实输出 + portal ScreenCast 需交互授权（headless 无输入确认）——需真实 Wayland 桌面会话（GNOME/KDE Wayland 或交互 weston）验证首帧/缩放 |
...
- **R5 落地清单（200 点）**：✔ 类约 **99%**（可靠通道 32 项 / 前端 23 项 / 抓帧 8 项 / 编码器 8 项 / 打包 5 项 / 遥测测试 18 项 / TestDelay 探针 1 项 / admin KPI 曲线 1 项 / 光标通道 1 项 / QoS 状态机 2 项 / 多会话隔离 1 项 / 重连矩阵 1 项 / SSE 空闲看门狗 1 项 / 弱网降级提示 1 项 / **#41/#43/#14 binary 线协议闭环 3 项（第 67-69 轮）+ #8/#67-70 收口**）；⬜ **~0%**（全部代码就绪，仅 #132-134 Wayland 完整捕获运行时验证待真实 Wayland 会话）；◐ **~2%**（#36-38 KCP 本体 / #44 老版本二进制兼容 / #48/#72 决策关闭 / #127-128 内存池 / #132-134 Wayland 运行时——决策关闭或架构阻塞）。第 76 轮 Wayland 编译就绪 + portal 集成就绪，R5 代码层面全就绪
- **第 13 轮新增合入**：
...
- **第 79 轮：SIMD 内存池第三块（捕获侧池注入环形闭环）**：
  1. `FrameSource::attach_pool` trait 默认 no-op（各 backend 无破坏）；X11Source 挂 `pool: Option<Arc<FramePool>>` 字段 + `bgra_from_pixels` 从池 `acquire` 借 bgra（无池自分配兜底）。
  2. `ThreadedFrameSource::spawn_with_max_fps` 加 pool 参数（`inner.attach_pool` 线程内注入）；pipeline 池改 `Arc<FramePool>` 共享——**capture 借出 → 主循环转换后归还 → capture 复用环形闭环**（capture+encode 双端池化）。
  3. **验证**：全量 `cargo test` **417 通过**（含 X11 捕获测试覆盖池化路径）；4-top 动态基准 **PASS**（fps 30.0 / bitrate 670kbps，捕获池化无回归）。
  4. **#127 三件套闭环**（FramePool 组件第 77 + 编码链接入第 78 + 捕获侧环形第 79）。完成后按用户指示转 **MYS-931**。
- **第 78 轮：SIMD 内存池第二块（FramePool 接入 pipeline）**：
  1. `pooled_frame(pool, w, h, bgra)` 辅助函数：从池借 buffer + 拷贝构建捕获帧（复用命中）。
  2. 接入：run_desktop_pipeline 创建 `FramePool::new(4)` + `prealloc(2, w0*h0*4)`；**合成帧路径**（静态 IDR/reqkey 重编 `mod.rs:1133`）`bgra.clone()` → `pooled_frame`（池借出）；**动态帧/合成帧转换完成后** `frame_pool.release(take(fr.bgra))` 归还（cap 保留复用）。
  3. **验证**：`test_pooled_frame_allocates_from_pool`（预分配 buffer 命中复用、无新分配、内容/尺寸正确）；全量 `cargo test` **417 通过**（+1）；4-top 动态基准 **PASS**（fps 30.0 / bitrate 670kbps，pipeline 池化无回归）。
  4. 第三块（捕获侧预分配 + 引用计数环形）为框架级后续；完成后转 MYS-931。
- **第 77 轮：SIMD 内存池第一块（FramePool buffer 池化组件）**：
  1. 用户指示（MYSY，2026-09-06）："做完 Wayland 后做 SIMD 内存池（架构级）。然后做 MYS-931"——Wayland 已就绪（第 76 轮编译+集成就绪），本轮启动 **#127 SIMD 内存池第一块**。
  2. **`FramePool` 组件落地**（desktop/mod.rs）：预分配复用 `Vec<u8>`——`new(capacity)` / `prealloc(n,size)` 启动摊分 / `acquire(size)` 池中借出（cap≥size 命中复用，否则新分配）/ `release(buf)` 归还（cap 保留、池满丢弃防膨胀）/ `stats()` 返回（复用, 新分配）原子计数可观测命中率。
  3. **验证**：3 单测——`test_frame_pool_reuses_buffers`（同尺寸循环借出 3 次仅 1 次新分配）/ `test_frame_pool_capacity_capped`（池限 2、第 3 借新分配）/ `test_frame_pool_grows_for_larger_size`（小池借大尺寸不硬塞）；全量 `cargo test` **416 通过**（+3）。
  4. 后续：frame-ring 完整接入（捕获侧预分配 + 编码侧归还 + 引用计数）为框架级演进，逐轮推进；完成后按用户指示转 MYS-931。
- **第 76 轮：Wayland #132-134 代码级就绪 + 集成路径验证**：
  1. **编译打通**：`wayland.rs`（zbus + gstreamer portal ScreenCast → PipeWire 完整实现）`--no-default-features --features wayland` **编译通过**（本机 gstreamer 1.28.6 pkg-config 就绪）——**代码级就绪**（此前从未编译验证）。
  2. **Wayland 测试栈搭建验证**：pacman 装 weston + xdg-desktop-portal + xdg-desktop-portal-wlr（gst-plugin-pipewire 已装）→ headless weston（`--backend=headless-backend.so --width=1280 --height=800 --socket=wtest0`）socket 起 → portal 加载 **wlr.portal 注册 ScreenCast**（日志确认）→ agent `--desktop-capture wayland` **portal 流程正常执行**（注册/desktop:start/WS uplink，无崩溃无错误）。
  3. **运行时验证阻塞（如实标注）**：headless weston 无真实输出 + portal ScreenCast 需交互授权（headless 无输入确认弹窗）——完整首帧/缩放验证需**真实 Wayland 桌面会话**（GNOME/KDE Wayland 或交互 weston + autorun portal）。
  4. 收口：#132-134 标 ◐（代码+集成路径就绪，运行时验证待真实 Wayland 环境）；**R5 落地清单代码层面全就绪**。全量 `cargo test` **413 通过**（无 wayland feature 默认构建不受影响）。
...
- **第 69 轮新增合入**：
  1. agent 发送侧 binary 化（R5 #41 全链路闭环）：relay `agent_send_handler` register 响应带 **`desktop_binary: true`** 能力告知；agent `RelayClient` 新增 `desktop_binary` 字段 + `parse_desktop_binary` 解析（缺省 false，老 relay 兼容回退 JSON+base64）。agent WS 上行改发：**ctrl 消息合并 text JSON 数组帧 + desktop:video 逐帧 binary**（`encode_bin_frame`，init→`BIN_FRAME_FLAG_INIT` / key→`BIN_FRAME_FLAG_KEY`；老 relay 无字段 → 原 JSON 数组 text；WS 失败仍回退 HTTP 批量 JSON——三态兼容）。**验证**：`test_parse_desktop_binary_capability`（缺省 false / true 启用 / 非 bool 容错）；**端到端实测**（relay debug）：agent→relay 全 binary——relay recv 195 帧 / `video-bin pushed(frag)` 97 / **浏览器 WS 收完整 fMP4（ftyp=1 + moof=29）**；`tools/bench_top4_verify.sh` PASS（binary 路径 fps 中值 30.0 满帧 / bitrate 670kbps）；全量 `cargo test` **413 通过**（+1）。**协议膨胀 0%** vs 此前 JSON base64 ≈33%（`desktop_proto` encoded==decoded）。#41 从"格式规格/消费侧"推进到**全链路闭环**：agent 编码端 binary → relay Binary 分支直转 → 浏览器 WS 原样 fMP4。
- **第 68 轮新增合入**：
  1. relay binary 帧消费侧落地（R5 #41 binary 传输推进）：`handle_agent_ws_uplink` 新增 `Message::Binary` 分支（此前 binary 帧被忽略）→ `route_bin_desktop_frame(state, sid, flags, seq, payload)`——**init 帧**（`BIN_FRAME_FLAG_INIT`，第 68 轮补该位标记）`set_init` 缓存重放新 viewer；**frag 帧** `push_frag` + 缓冲满限频回传 `desktop:congested`；seq 丢帧检测 + `desktop_proto` 开销统计（binary 原样字节 `encoded==decoded`，**0% 协议膨胀**——与 JSON base64 ≈33% 对照）；损坏帧（`decode_bin_frame` 返回 None）丢弃并 warn。老 agent 仍走 Text JSON 路径，relay 双通道并存。**验证**：`test_bin_desktop_frame_init_and_frag`（init 缓存 + 新 viewer 拿到 + frag 达 viewer 缓冲）+ `test_bin_desktop_frame_seq_drop_and_congested`（seq 跳变丢帧检测 + 缓冲满 congested 回传 + `encoded==decoded` 膨胀断言）；全量 `cargo test` **412 通过**（+2）。agent 发送侧切换（JSON→binary）为剩余远期（上行批处理重构 + HTTP 回退兼容）。
- **第 67 轮新增合入**：
  1. desktop:video 二进制帧格式规格（R5 #41 线协议最小可验证子集）：`proto/mod.rs` 新增 `encode_bin_frame`/`decode_bin_frame`——格式 `[flags:u8][seq:u32 LE][len:u32 LE][payload]`（`BIN_FRAME_FLAG_KEY` 位 0 = key/delta，`BIN_FRAME_HEADER_LEN`=9）。`len` 与 WS 帧边界**双重校验**：头不全 / 长度不符 / 篡改 len → `decode_bin_frame` 返回 None 不 panic（relay 对损坏帧直接丢弃）。payload = 原始 fMP4 init/frag 字节（与浏览器桌面 WS 下行同构，未来 binary 化 agent→relay 段时字节直转、消除 base64 ≈33% 膨胀）。**当前状态如实标注**：格式规格 + 双向编解码就绪（agent/relay 复用），实时传输仍走 JSON+base64——agent→relay WS binary 帧 + relay Binary 分支直转为架构级远期（本格式为其同构迁移目标）。**验证**：`test_bin_frame_roundtrip`（key/delta、seq 0 与 u32::MAX、空 payload）+ `test_bin_frame_rejects_malformed`（<9 字节、payload 截断/多出、len 篡改超大不 panic）；全量 `cargo test` **410 通过**（+2）。
- **第 66 轮新增合入**：
...
- **未合入（如实）**：线协议二进制整块（批次2 #41-44 的 binary 传输层）、跨进程时间线遥测完整版、独立 100ms ack 批（#48 决策关闭）、i444（编码器级远期——openh264 不支持，需 libaom i444 + 颜色转换改造）、**AV1 测速门槛已核实由 #85 覆盖**（`next_degrade_codec` 慢帧/Err 实际性能驱动降级 av1→vp9→h264，替代预测测速，决策关闭）——在台账对应 ⬜/◐ 行，未宣称完成。