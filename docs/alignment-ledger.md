...
- **R4/5（200 点）**：✔ 类约 **49%**（甲 帧率/IDR、乙 Player 主体+韧性+错误恢复+内存+排队归因、丁 弱网可见性+输入节流、丙 遥测基线+日志+分辨率事件+带宽记账）；⬜ 25%（丙遥测剩余、戊发布门槛）；◐ 26%。
- **R5 落地清单（200 点）**：✔ 类约 **52%**（可靠通道 9 项 / 前端 20 项 / 抓帧 6 项 / 编码器 2 项 / 打包 3 项 / 遥测测试 8 项）；⬜ 32%。
- **第 11 轮新增合入**：
  1. 未知消息白名单丢弃（`relay/ws.rs` route_agent_message 末尾 KNOWN 白名单外丢弃+日志，防垃圾/未知控制消息静默吞掉）→ R3 丙137 / R5#21；
  2. 桌面流 map 生命周期追踪（`relay/ws.rs` created/removed 带原因日志：desktop:started/stopped/agent断线，实测三路径完整，重复 stopped 幂等 existed=false）→ R2 丙129 / R5#24。
