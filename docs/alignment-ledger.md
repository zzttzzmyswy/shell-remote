...
| # | 点 | 状态 | 证据 |
|---|---|---|---|
| 41 | 逐帧 binary+头{len,seq,flags} | ◐ | **帧头字段显式化已做**（第 52 轮：desktop:video frag 带消息级 `seq`（递增帧号，与 fMP4 moof seqn 同步）+ `flags`（key/delta）——逐帧 binary 头的 JSON 等价子集，未来 binary 化可同构迁移；`pipeline_static` 测试断言）；**relay 侧 seq 丢帧检测已做**（第 55 轮：`desktop_proto` 统计加 last_seq/dropped——消息级 seq 跳变检测 agent→relay 段丢帧，与浏览器段 moof seqn 互补；测试 `test_desktop_seq_gap_detects_dropped_frames`）；**binary 帧格式规格已落地**（第 67 轮：`proto/mod.rs` `encode_bin_frame`/`decode_bin_frame`——`[flags:u8][seq:u32 LE][len:u32 LE][payload]`，`BIN_FRAME_FLAG_KEY`/`BIN_FRAME_FLAG_INIT`（第 68 轮补 init 标记），len 与 WS 帧边界双重校验（截断/损坏拒绝）；测试 `test_bin_frame_roundtrip`（key/delta/init/seq 边界/空 payload）+ `test_bin_frame_rejects_malformed`（头不全/长度不符/篡改 len 不 panic））；**relay 消费侧已落地**（第 68 轮：`handle_agent_ws_uplink` 加 `Message::Binary` 分支 → `route_bin_desktop_frame`——init 帧 set_init 缓存重放 / frag 帧 push_frag + 限频回传 congested / seq 丢帧检测 + 开销统计（binary encoded==decoded，0% 协议膨胀）与 text 路径同语义；测试 `test_bin_desktop_frame_init_and_frag`（init 缓存+新 viewer 拿到+frag 达 viewer）+ `test_bin_desktop_frame_seq_drop_and_congested`（seq 跳变检测+缓冲满回传+膨胀断言））；**agent 发送侧切换（JSON→binary）仍远期**（agent 上行批处理重构 + HTTP 回退兼容） |
...
- **R4/5（200 点）**：✔ 类约 **78%**（甲 帧率/IDR/RTT分带/TestDelay探针/QoS五态/编码降级链、乙 Player 主体+韧性+30s判死+能力回退链+重连降质+内存+排队归因+光标叠加+解码器释放、丙 遥测基线+日志+分辨率事件+带宽记账+admin KPI 曲线+归因决策树+告警雏形+统一时间线工具、丁 弱网可见性+输入节流+RTT分带+重连窗口降质量、戊 验收脚本化全闭环）；⬜ 12%（线协议 binary 传输 agent 侧 #41-43、KCP、DXGI/GDI Windows、Wayland、i444、SIMD 内存池部分）；◐ 10%（部分依赖架构远期）。第 68 轮合计实（relay binary 消费侧落地，R5 99% 保持）。
