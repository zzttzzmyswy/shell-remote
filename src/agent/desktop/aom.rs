...
/// AV1 realtime `cpu_used` 固定 5（MYS-954 三轮档位实测 5-10 后的最终
/// 取值）。首轮 5-10 扫描：speed 6 是带宽优先点（实际码率 436kbps，目标
/// 1M 下省 56%），speed 7-9 码率飙到 1.2-1.4M 超带宽预算，speed 10 画质
/// 崩塌（SSIM 0.50）——当时选 6。第二轮用户反馈 speed 6 在 1080p 高熵
/// 下编码耗时偏高（86ms/帧）致掉帧花屏，调 8（70ms/帧）缓解，但 speed 8
/// 画质偏糙、码率回升到目标档。第三轮用户最终定档 **speed 5**：更精细
/// 的编码决策（画质最好档，码率也最省），单帧耗时最高（高熵下约
/// 100ms+）交给 QoS 动态降帧兜底——用户环境优先画质/码率，接受高熵下
/// 的帧率自适应。realtime 档下再开 intrabc/rect 等工具被 speed 档绑定、
/// 无额外增量，故只调 speed 不开工具）。
///
/// 原面积分档（对齐 rustdesk webrtc `get_cpu_speed`）在 1080p 给 10，
/// 高 speed 优先保帧率但牺牲画质；固定 5 是"画质/码率最优、帧率交给
/// QoS"的取舍。
...
impl AomEncoder {
    ...
    pub fn new(
        ...
        unsafe {
            ...
            //   - cpu_used 固定 5（MYS-954 三轮定档：5=画质/码率最优、帧率
            //     交 QoS 降帧；6 曾因高熵掉帧花屏、8 画质糙，见 av1_cpu_used）
...
mod tests {
    ...
    /// cpu_used 固定 5（MYS-954 三轮定档：5=画质/码率最优、帧率交 QoS；
    /// 6 曾高熵掉帧花屏、8 画质糙）；superblock 仍按面积/线程分档（#81，
    /// 对齐 rustdesk get_super_block_size）。
    #[test]
...
mod bench_encode_time {
    ...
    fn bench_av1_1080p_complex_encode_ms_per_frame() {
        ...
        // cpu_used=5 + screen tune + 禁高耗时工具后 1080p 高熵单帧
        // 单独跑约 170ms（speed 5 是精细档，比 6/8 明显更慢）。上限 400ms
        // 是宽松护栏（并行跑全量测试时 CPU 竞争可把单帧拖到 300ms+）；
        // 真正的延迟验证在 E2E 链路完成。
        assert!(ms < 400.0, "av1 frame took {ms:.1}ms — latency bug remains");
    }
