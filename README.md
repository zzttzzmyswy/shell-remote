...
- **连接自愈** — 设备已注册但 agent 链路未建立或断开时不再僵死在空白终端：终端页提示"设备连接中断，正在自动重连"并自动重连，agent 恢复后终端自动回放续上
...
- **设备管理**：每台 agent 启动/重连时自动探测并上报主机信息（主机名、架构、系统、内核、CPU 型号）。"设备"面板逐台展示，支持按关键字（会话ID/主机名/架构/系统/CPU）、架构、在线状态筛选，并可一键连接该设备终端。未上报设备信息的旧版 agent 也能正常列出。
